...
To reuse the default struct you need derive the MTKTokenState trait and mark the corresponding field with the #[MTKStateKeeper] attribute.

You can also add your fields in your MTK contract. For example, let's add the owner's address to the contract, the token_id that will track the current number of token and the supply to store how many of differnt tokens were minted:

要重复使用默认结构，你需要派生出 `MTKTokenState` ，并用 `#[MTKStateKeeper]` 属性标记相应的字段。你也可以在 MTK 合约中添加字段。例如，在合约中添加所有者的地址，`token_id` 将跟踪当前的代币数量，供应量储存不同代币的铸造数量。
