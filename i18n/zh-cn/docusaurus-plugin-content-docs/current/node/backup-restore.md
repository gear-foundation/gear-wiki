...
节点可以连接到不同的链。可以使用 `--chain` 参数来选择链。目前默认的链是 staging test 网络。它的数据位于 `gear-node/chains/staging_testnet_v4`目录中。
...
- 节点的网络密钥：
    ...
    - Windows: `%USERPROFILE%\AppData\Local\gear-node.exe\chains\staging_testnet_v4\network\secret_ed25519`
...
- (可选) 数据库：
    ...
    - Windows: `%USERPROFILE%\AppData\Local\gear-node.exe\chains\staging_testnet_v4\db\full`
