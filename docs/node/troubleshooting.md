...
- **Error:** `IO error: While lock file /root/.local/share/gear/chains/gear_stable_testnet/db/full/LOCK: Resource temporarily unavailable`
...
- **Solution:** The root of this problem is the lack of the disk free space. You may check the free space using the following command:
    ...
    ```shell
    du -h $HOME/.local/share/gear/chains/gear_stable_testnet/db/full
    ```
