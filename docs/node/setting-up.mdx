...
Gear node cannot works correctlly on QEMU Virtual CPU because support of AVX and SSE 4.2 is required.

Other than that, Gear node doesn't have any special hardware requirements excepting SSD.
...
**macOS M1/M2**: [gear-nightly-macos-m1.tar.gz](https://builds.gear.rs/gear-nightly-macos-m1.tar.gz)
...
You can find the final build at the following location: `gear/target/release/gear-node`
...
```code
RUST_LOG="gwasm=debug" gear-node [subcommand] [options]
```
