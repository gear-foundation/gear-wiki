...
```bash
docker pull ghcr.io/gear-dapps/gear-env:1.0
```
...
```bash
docker run --rm --name gear-env -itd ghcr.io/gear-dapps/gear-env:1.0 bash
```
</TabItem>
...
```bash
docker run --rm --name gear-env --platform linux/amd64 -itd ghcr.io/gear-dapps/gear-env:1.0 bash
```
</TabItem>
</Tabs>
````
