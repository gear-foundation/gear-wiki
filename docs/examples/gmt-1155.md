...
Saves the user's balances and approvals. Contains commands to manipulate these fields. When the user interacts with `mt-logic`, a new associated `mt-storage` is created - this provides scalability and greatly increases memory limits.
