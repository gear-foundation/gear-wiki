...
As result of the compilation of the conract, will be generated `mata.txt` a file that can be used in future UI applications. And looks like a Hex:
...
To make it possible to verify metadata for a program, you can use `metahash()` function. It allows verify metadata on-chain.

```rust
#[no_mangle]
// It returns the Hash of metadata. 
// .metahash is generating automatically while you using build.rs
